...
export default function BusinessReportPage() {
  ...
  const availableYears = Array.from(new Set(
    disclosures?.filter(d => d.disclosureType === 'business-report' && d.isVisible)
  ...
  const handleView = (pdfUrl: string) => {
    if (!pdfUrl || pdfUrl.trim() === '') {
      toast({
        ...
        description: language === 'jp' ? 'この事業報告書にはPDFファイルが添付されていません。' : 'This business report does not have a PDF file attached.',
        variant: "destructive",
      });
      return;
    }
            ...
            <div className="bg-white/10 backdrop-blur-sm rounded-lg p-6">
              <div className="mb-4">
                ...
                <Select value={selectedYear.toString()} onValueChange={(value) => setSelectedYear(parseInt(value))}>
                  <SelectTrigger className="w-full bg-white/20 border-green-300 text-white">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button 
                        onClick={() => {
                          ...
                          handleDownload(disclosure.pdfUrl, language === 'jp' ? '事業報告書' : 'Business Report');
                        }}
